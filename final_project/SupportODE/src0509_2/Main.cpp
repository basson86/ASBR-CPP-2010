...
void CreateWorld1(Simulator * sim)
{
    ...
    geom_lot1 = dCreateBox(sim->GetSpaceID(), 8, 0.2, zdim);              
    dGeomSetPosition(geom_lot1, -5, -4.5, 0.5 * zdim);  
    ...
    geom_lot1 = dCreateBox(sim->GetSpaceID(), 1.3, 0.2, zdim);              
    dGeomSetPosition(geom_lot1, -0.5, -4.5, 0.5 * zdim);  
    dQFromAxisAndAngle(q, 0, 0, 1, M_PI * 1.5);
    dGeomSetQuaternion(geom_lot1, q);
    sim->AddGeometryAsObstacle(geom_lot1);//*/


    geom = dCreateBox(sim->GetSpaceID(), 8, 0.2, zdim);              
    dGeomSetPosition(geom, 3.5, -4.5, 0.5 * zdim);  
    dQFromAxisAndAngle(q, 0, 0, 1, M_PI * 1.5);
    dGeomSetQuaternion(geom, q);
    sim->AddGeometryAsObstacle(geom);


	geom = dCreateBox(sim->GetSpaceID(), 12, 0.2, zdim);              
    dGeomSetPosition(geom, 1, -1, 0.5 * zdim);  
    dQFromAxisAndAngle(q, 0, 0, 1, M_PI * 1);
    dGeomSetQuaternion(geom, q);
    sim->AddGeometryAsObstacle(geom);
