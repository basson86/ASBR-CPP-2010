...
//create robot
    ...
    dBodySetPosition(m_robotBodyChassis, posx, posy, 0.85 * RADIUS + 0.5 * HEIGHT + STARTZ);
    ...
    dBodySetPosition(m_robotBodyWheels[0],  posx + 0.5 * LENGTH, posy,               RADIUS + STARTZ);
    dBodySetPosition(m_robotBodyWheels[1],  posx - 0.5 * LENGTH, posy + 0.5 * WIDTH, RADIUS + STARTZ);
    dBodySetPosition(m_robotBodyWheels[2],  posx - 0.5 * LENGTH, posy - 0.5 * WIDTH, RADIUS + STARTZ);
...
//set desired speed    
    ...
    dJointSetHinge2Param(m_robotJoints[0], dParamFMax2, 0.1);   // maximum torque = 0.1
