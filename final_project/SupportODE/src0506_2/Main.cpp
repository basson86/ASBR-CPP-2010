...
void CreateWorld1(Simulator * sim)
{
    ...
    geom_gara1 = dCreateBox(sim->GetSpaceID(), 2.5, 0.2, zdim);              //2.5
    dGeomSetPosition(geom_gara1, -13.75 + 0.5*(30-xdim), -5.5, 0.5 * zdim);  // -13.75 + 0.5*(30-xdim)
	...
	geom_gara2 = dCreateBox(sim->GetSpaceID(), 2.5, 0.2, zdim);              //2.5
    dGeomSetPosition(geom_gara2, -13.75+ 0.5*(30-xdim), -8, 0.5 * zdim);     // -13.75 + 0.5*(30-xdim)
    dQFromAxisAndAngle(q, 0, 0, 1, M_PI * 1);
