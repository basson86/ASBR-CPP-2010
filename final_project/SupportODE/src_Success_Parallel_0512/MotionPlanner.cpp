...
while (m_vidGoal!=1)
{
    ...
    residue_error = sqrt(pow((FOvertex->m_state[0]-ParkingLotState[0]),2) + pow((FOvertex->m_state[1]-ParkingLotState[1]),2) + pow((FOvertex->m_state[6]-ParkingLotState[6]),2));


	if(residue_error<= 1.15) //1.15,0.8
	{m_vidGoal=1;}

    printf("residual error = %f, size of T = %d \n", residue_error, m_vertices.size());
 ...
 if ( m_config_grid[qxI][qyI][qtheI]==0 && m_vidGoal!= 1)
  {
	...
	{ 
	  ...
	  { //nL_state->cost =m_vertices[nL_state->m_parent]->cost + 5;
        nL_state->cost =m_vertices[nL_state->m_parent]->cost + 10;  //10
	  }
	...
	{ 
      ...
      { //nR_state->cost =m_vertices[nR_state->m_parent]->cost + 5;
		nR_state->cost =m_vertices[nR_state->m_parent]->cost + 10; //10
	  }
	...
	{ 
	  ...
	  { //nS_state->cost =m_vertices[nS_state->m_parent]->cost + 5;
        nS_state->cost =m_vertices[nS_state->m_parent]->cost + 10; //10
	   }
...
//*************************************************************************************************
	...
	if (m_simulator->SimulateOneStep(forward_factor*step_speed, -step_steer))
	{ 
      //printf("collision free\n");
	  ...
	  { pL_state->cost =m_vertices[pL_state->m_parent]->cost + 5;} //5
	  else 
	  { //pL_state->cost =m_vertices[pL_state->m_parent]->cost + 5;
	    pL_state->cost =m_vertices[pL_state->m_parent]->cost + 10;  //10
	  }
	...
	{ 
	  ...
	  { pR_state->cost =m_vertices[pR_state->m_parent]->cost + 5;}  //5
	  else 
      { //pR_state->cost =m_vertices[pR_state->m_parent]->cost + 5;
	    pR_state->cost =m_vertices[pR_state->m_parent]->cost + 10;   //10
	  }
	...
	if (m_simulator->SimulateOneStep(forward_factor*step_speed, 0))
	{ 
	  ...
	  { pS_state->cost =m_vertices[pS_state->m_parent]->cost + 5;}  //5
	  else 
      {   //pS_state->cost =m_vertices[pS_state->m_parent]->cost + 5;
         pS_state->cost =m_vertices[pS_state->m_parent]->cost + 10;  //10
	  }
   ...
   /*k++;
	if (k== 80000 )//|| m_vertices.size()>= 150000)//85000 ) // 1477,14173,15107,1181
   	{m_vidGoal=1;}  //*/
