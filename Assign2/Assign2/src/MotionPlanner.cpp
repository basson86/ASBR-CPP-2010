...
MotionPlanner::MotionPlanner(Simulator * const simulator)
{
	...
	Resolution= 0.7;  // the resolution of edge for collision check
...
// push_back the computed "Path" into the "path" for graphic visualization
   ...
   {  
	   for (int j=0; j<nd ; j++)
	   {
	   	 path->push_back((1-j/nd)*Path[3*i]+(j/nd)*Path[3*(i+1)]);
         path->push_back((1-j/nd)*Path[3*i+1]+(j/nd)*Path[3*(i+1)+1]);
	     path->push_back((1-j/nd)*Path[3*i+2]+(j/nd)*Path[3*(i+1)+2]);
